...
def getPostFitUncFromCard(cardFile, estimateName, uncName, bin):
    nuisanceFile = cardFile.replace('.txt','_nuisances_full.txt')
    return getFittedUncertainty(nuisanceFile, estimateName)*getPreFitUncFromCard(cardFile, estimateName, uncName, bin)

def getPreFitUncFromCard(cardFile, estimateName, uncName, bin):
    with open(cardFile) as f:
      ...
      raise Warning('No uncertainty for ' + estimateName + ' bin ' + str(bin))
...
def applyNuisance(cardsFile, estimate, res, bin):
    ...
    scaledRes  = res*(1+getPreFitUncFromCard(cardsFile, estimate.name, estimate.name, bin)*getPull(nuisanceFile, estimate.name))
