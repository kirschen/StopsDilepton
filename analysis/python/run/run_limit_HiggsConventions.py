...
def wrapper(s):
    ...
    if not os.path.exists(cardFileName) or overWrite:
        ...
        c.addUncertainty('CMS_btag_heavy',          'lnN')
        c.addUncertainty('CMS_btag_light',          'lnN')
        c.addUncertainty('CMS_eff_trigger',         'lnN')
        c.addUncertainty('CMS_eff_l',               'lnN')
        c.addUncertainty('QCDscale_ttH_ACCEPT',     'lnN')
        ...
        for setup in setups:
          eSignal     = MCBasedEstimate(name="ttH_hinv", sample={channel:s for channel in channels+trilepChannels}, cacheDir=setup.defaultCacheDir())
                ...
                for e in setup.estimators:
                  ...
                  if e.name.count('TTJets'):
                    if len(setup.regions) == len(regionsO[1:]):     divider = 6
                        ...
                        if int(args.popFromSR) < 1 and args.popFromSR is not False: divider = 0
