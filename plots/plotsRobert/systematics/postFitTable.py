...
for tex_name, name, sys_names in systematics:
    ...
    for sys_name in sys_names:
        sys[sys_name] = Results.getNuisanceYields(sys_name, postFit=False)
    ...
    logger.info( "Systematic %20s. variation quantiles 50 / 68 / 90 / max %4.3f %4.3f %4.3f %4.3f", name, \
...
"""
\\begin{{table}}
   \caption{{Typical values (90\% quantiles) of the post-fit variations of all signal regions.}} \label{{tab:post-fit}}
   \center
      \\begin{{tabular}}{{r|c|r|c}}
            systematic  & \% & systematic &\% \\\\\\hline
{systematics}
      \end{{tabular}}
\end{{table}}
""".format(systematics = "\n".join(pair_strings))
