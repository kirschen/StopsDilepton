...
argParser.add_argument('--mode',           action='store',      default='3mu',      choices=['3mu', '3e', '2mu1e','2e1mu'])
argParser.add_argument('--plot_directory', action='store',      default='TTZ')
argParser.add_argument('--selectPrefix',   action='store',      default=None)
...
#
# Make samples, after defining the postProcessing directory
#
...
#
# Three lepton selection (at least two good with pt > 20 GeV, third can be loose with pt > 10 GeV)
#
...
emuSelection    = "(" + ")||(".join([mumueSelection, mueeSelection]) + ")"
...
#
# To add some isolation constraints (not really optimal, just here to do some tests, currently not used)
#
def getDeltaRString(type1, index1, type2, index2, dR):
  eta1 = type1 + "Good_eta[" + str(index1) + "]"
  eta2 = type2 + "Good_eta[" + str(index2) + "]"
  phi1 = type1 + "Good_phi[" + str(index1) + "]"
  phi2 = type2 + "Good_phi[" + str(index2) + "]"
  return "sqrt((" + eta1 + "-" + eta2 + ")**2 + (" + phi1 + "-" + phi2 + ")**2)>=" + str(dR)

def getLeptonDeltaRString(index1, index2):
  return getDeltaRString("Lep", index1, "Lep", index2, 0.1)

def getJetDeltaRString(lepIndex, jetIndex):
  return getDeltaRString("Lep", lepIndex, "Jet", jetIndex, 0.4)

lepIsolation    = getLeptonDeltaRString(0, 1) + "&&" + getLeptonDeltaRString(1, 2) + "&&" + getLeptonDeltaRString(0, 2)
jetIsolation3   = "&&".join([getJetDeltaRString(i, j) for i in range(3) for j in range(3)])
jetIsolation4   = "&&".join([getJetDeltaRString(i, j) for i in range(3) for j in range(4)])
...
cuts=[
    ...
    ("nbtag2",            bJetSelectionM+"1&&"+bJetSelectionL+"2"),
    ("mll20",             "dl_mass>20"),
    ("met80",             "met_pt>80"),
...
# TODO: try to fill together for the difference selection cuts
#                
for i_comb in reversed( range( len(cuts)+1 ) ):
    for comb in itertools.combinations( cuts, i_comb ):
        ...
        plot_path = os.path.join(plot_directory, args.plot_directory, args.mode)
        if args.leptonsExact: plot_path += "_noAdditionalLeptons"
        if os.path.exists(plot_path) and not args.overwrite:
            logger.info( "Path %s not empty. Skipping."%path )
            continue

        if args.selectPrefix is not None and args.selectPrefix != prefix: continue 
        if "nbtag1" in prefix and "nbtag2" in prefix: continue
        if "njet3"  in prefix and "njet4"  in prefix: continue
        if not "njet" in prefix: continue
        ...
        # Use some defaults
        Plot.setDefaults(stack = stack, weight = weight, selectionString = selectionString, prefix = prefix)
