...
if args.year == 2016:
    ...
    weight_string = 'weight*reweightLeptonTrackingSF*reweightBTag_SF*reweightLeptonSF*reweightDilepTrigger*reweightPU*reweightL1Prefire*%f'%lumi_scale16
    weight = 'weight'
elif args.year == 2017:
    from StopsDilepton.samples.nanoTuples_Run2017_31Mar2018_postProcessed import *
    from StopsDilepton.samples.nanoTuples_Fall17_postProcessed import *
    #samples             = [ multiBoson_17]
    samples             = [ TTZ_17, TTXNoZ_17, multiBoson_17, DY_HT_LO_17, Top_pow_17]
    data_sample17 = eval('Run2017')
    lumi_scale17                 = data_sample17.lumi/1000
    samples17 = [data_sample17] + samples
    weight_string = 'weight*reweightLeptonTrackingSF*reweightBTag_SF*reweightLeptonSF*reweightDilepTrigger*reweightPU*reweightL1Prefire*%f'%lumi_scale17
    weight = 'weight'
else:
    from StopsDilepton.samples.nanoTuples_Run2018_PromptReco_postProcessed import *
    from StopsDilepton.samples.nanoTuples_Autumn18_postProcessed import *
    #samples             = [ multiBoson_18]
    samples             = [ TTZ_18, TTXNoZ_18, multiBoson_18, DY_HT_LO_18, Top_pow_18]
    data_sample18 = eval('Run2018')
    lumi_scale18                 = data_sample18.lumi/1000
    samples18 = [data_sample18] + samples
    weight_string = 'weight*reweightLeptonTrackingSF*reweightBTag_SF*reweightLeptonSF*reweightDilepTrigger*reweightPUVUp*reweightL1Prefire*reweightHEM*%f'%lumi_scale18 #FIXME! no PUVUP
    weight = 'weight*reweightHEM'
...
path = "/afs/hephy.at/user/p/phussain/www/stopsDilepton/analysisPlots/cutFlow/v03/%i"%args.year
...
for i_cut, cut in enumerate(cuts):
    ...
    for i_sample, sample in enumerate(samples):
       ...
       #     values[i_cut][i_sample] = sample.getYieldFromDraw(selectionString = "&&".join([ '('+c[2]+')' for c in cuts[:i_cut+1]]), weightString = weight_string16)['val'] 
        if "Run" in sample.name:
            values[i_cut][i_sample] = sample.getYieldFromDraw(selectionString = "&&".join([ '('+c[2]+')' for c in cuts[:i_cut+1]]), weightString = weight)['val']
            print sample.name
        else:
