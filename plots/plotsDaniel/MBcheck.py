...
#selection = "(isMuMu)&&nJetGood>1&&dl_mt2ll>100&&nBTag==0&&met_pt>80&&metSig>5&&l1_pt>25&&l2_pt>20&&l1_relIso03<0.12&&l2_relIso03<0.12&&abs(dl_mass-91.2)<15"
...
mc_selection = "&&".join([selection, getFilterCut(isData=False)])
...
mc = [DY_HT_LO, multiBoson, Top_pow, TTZ, TTXNoZ]

binning = [0,20,40,60,80,100,120,140,160,200,250,300,350]

for s in mc:
    print s.name
    histos[s.name] = s.get1DHistoFromDraw("dl_mt2blbl", binning, binningIsExplicit=True, selectionString=mc_selection, weightString=weight, addOverFlowBin='upper')
...
plots_CR = [ [histos[s.name] for s in mc], [histos['data'] ] ]

plotting.draw(
    Plot.fromHisto("dl_mt2blbl",
                plots_CR,
                texX = "M_{T2}(blbl) (GeV)",
                texY = "Events"
            ),

    plot_directory = os.path.join(plot_directory, "MB_test_altBinning_EE"),
