...
T2tt5                   = T2tt_500_375
T2tt6                   = T2tt_500_400
#T2tt.style              = styles.lineStyle( ROOT.kOrange, width=3)
T2tt2.style             = styles.lineStyle( ROOT.kBlue, width=3 )
T2tt3.style             = styles.lineStyle( ROOT.kRed, width=3 )
T2tt4.style             = styles.lineStyle( ROOT.kGreen, width=3 )
T2tt5.style             = styles.lineStyle( ROOT.kCyan, width=3 )
T2tt.style             = styles.lineStyle( ROOT.kMagenta, width=3 )

T2bW                    = T2bW_250_1
T2bW2                   = T2bW_250_50
T2bW3                   = T2bW_300_1
T2bW.style              = styles.lineStyle( ROOT.kOrange, width=3)
T2bW2.style             = styles.lineStyle( ROOT.kBlue, width=3 )
T2bW3.style             = styles.lineStyle( ROOT.kRed, width=3 )


#signals = [  ]
signal1 = [ T2bW ]
signals = [ T2tt ] 
#signals = [ T2bW2, T2bW3 ]#, T2tt4, T2tt5, T2tt6 ]

postProcessing_directory = "postProcessed_80X_v35/dilepTiny"
from StopsDilepton.samples.cmgTuples_FullSimTTbarDM_mAODv2_25ns_postProcessed import *
from StopsDilepton.samples.cmgTuples_FullSimTTbarDM_mAODv2_25ns_NLO_postProcessed import *
DM                      = TTbarDMJets_DiLept_pseudoscalar_NLO_Mchi_10_Mphi_10
DM.texName              += " NLO"
DM2                     = TTbarDMJets_DiLept_pseudoscalar_Mchi_10_Mphi_10
...
for index, mode in enumerate(allModes):
  ...
  for sample in DMsignals + DMsignals2:
        sample.scale          = lumi_scale
  ...
  #tmp = [s for s in signals] + [s for s in DMsignals]
  #tmp = [[s] for s in DMsignals]

  #stack = Stack( DMsignals  )
  #stack.extend( [ [s] for s in DMsignals2 ])
  ...
  plots.append(Plot(
    texX = '#Delta#phi(ll, E_{T}^{miss})', texY = 'Number of Events',
    name = 'deltaPhillMet',
    attribute = lambda event, sample: acos( cos( event.dl_phi - event.met_phi )),
    read_variables = ["met_phi/F", "dl_phi/F"],
    binning = [16,0.,3.2],
  ))
  
  plots.append(Plot(
    texX = '#Delta#phi(l1, E_{T}^{miss})', texY = 'Number of Events',
    name = 'deltaPhil1Met',
    attribute = lambda event, sample: acos(cos( event.l1_phi - event.met_phi )),
    read_variables = ["met_phi/F", "l1_phi/F"],
    binning = [16,0,3.2],
  ))

  plots.append(Plot(
    texX = '#Delta#phi(l2, E_{T}^{miss})', texY = 'Number of Events',
    name = 'deltaPhil2Met',
    attribute = lambda event, sample: acos(cos( event.l2_phi - event.met_phi )),
    read_variables = ["met_phi/F", "l2_phi/F"],
    binning = [16,0,3.2],
  ))

  plots.append(Plot(
    texX = '#Delta#phi(l1, l2)', texY = 'Number of Events',
    name = 'deltaPhil1l2',
    attribute = lambda event, sample: acos(cos( event.l1_phi - event.l2_phi )),
    read_variables = ["l1_phi/F", "l2_phi/F"],
    binning = [16,0.,3.2],
  ))
  ...
  if args.selection.count('njet2') or args.selection.count('njet1'):
    ...
    plots.append(Plot(
      texX = 'p_{T}(jet) (GeV)', texY = 'Number of Events',
      name = 'jet_pt', attribute = lambda event, sample: event.JetGood_pt[0]*(event.JetGood_btagCSV[0]>0.800),
...
for mode in ["SF","all"]:
  ...
  dataMCScale = 1#yields[mode]["data"]/yields[mode]["MC"] if yields[mode]["MC"] != 0 else float('nan')
