...
eg. ipython -i -- simple_memoryint.py --selection njet2p-btag1p-relIso0.12-looseLeptonVeto-mll20-met80-metSig5-dPhiJet0-dPhiJet1 --small
'''
...
if not os.path.exists( os.path.join( h5_dir, store_dir ) ):
    os.makedirs( os.path.join( h5_dir, store_dir ))
...
df.drop(['label'], axis=1).to_hdf( os.path.join( h5_dir, store_dir) + 'data_X.h5', key='df', mode='w')
df['label'].to_hdf(  os.path.join( h5_dir, store_dir ) + 'data_y.h5', key='df', mode='w')
